...
live" - nothing downstream trusts a candidate this hasn't approved. A classifier candidate passes
when its accuracy is no more than validation_accuracy_tolerance below current's; a regressor
candidate passes when its RMSE is no more than validation_rmse_tolerance_ratio (a fraction of
current's RMSE) higher than current's.
"""
...
def validate_candidate(
    ...
    *,
    ...
    current_metrics = evaluate_model(
        current_model, X, y, framework=current_framework, estimator_type=estimator_type
    )
    candidate_metrics = evaluate_model(
        candidate_model, X, y, framework=candidate.framework, estimator_type=estimator_type
    )
    ...
    else:  # rmse - lower is better, tolerance scales with current's own RMSE
